...
export default function Sidebar({ activePage, onNavigate, collapsed, onCollapse, mobileOpen, onMobileClose }) {
    ...
    <aside className={`sidebar ${collapsed ? 'collapsed' : ''} ${mobileOpen ? 'mobile-open' : ''}`}>
      ...
      <button
        ...
        style={{
          ...
          zIndex: 60, boxShadow: '0 2px 8px rgba(44,31,20,.3)', cursor: 'pointer',
        }}
      >{collapsed ? '›' : '‹'}</button>
      ...
      {/* Footer */}
      <div style={{ padding: collapsed ? '14px 16px' : '14px 22px', borderTop: '1px solid rgba(201,169,110,.08)', display: 'flex', flexDirection: 'column', gap: '8px', flexShrink: 0 }}>
        {profile?.role === 'admin' && (
          <div style={{
