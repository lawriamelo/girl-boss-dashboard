...
function wkKey(offset = 0) {
  return wkStart(offset).toISOString().slice(0,10)
}

function wkLabel(offset = 0) {
  const s = wkStart(offset)
...
export default function Instagram() {
  ...
  async function saveNotes(val) {
    ...
    await supabase.from('instagram_weekly').upsert(
      { user_id: user.id, week_start: wk, notes: val, updated_at: new Date().toISOString() },
      { onConflict: 'user_id,week_start' }
    )
  }

  async function changeWk(dir) {
    const newOff = wkOffset + dir
    setWkOffset(newOff)
    await loadNotes(newOff)
  }
  ...
  async function savePost() {
    ...
    const payload = {
      ...
      impressoes: num(form.impressoes),
      salvamentos: num(form.salvamentos),
      seguidores_ganhos: num(form.seguidores_ganhos),
    }
    if (editId) {
  ...
  // Best post
  const bestPost = wkPosts.length > 0 ? wkPosts.reduce((best, p) => {
    const engaj = num(p.curtidas) + num(p.comentarios) + num(p.salvamentos)
    const bestEngaj = num(best.curtidas) + num(best.comentarios) + num(best.salvamentos)
    return engaj > bestEngaj ? p : best
  }, wkPosts[0]) : null
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: '22px', paddingBottom: '18px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <div style={{ fontSize: '8.5px', letterSpacing: '.3em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '3px' }}>Análise Semanal</div>
          <div style={{ fontFamily: 'var(--serif)', fontSize: '26px', fontWeight: 700, color: 'var(--mocha)' }}>Instagram</div>
        </div>
        <button onClick={() => { setForm(EMPTY_POST); setEditId(null); setModal(true) }} style={{ padding: '8px 20px', background: 'var(--mocha)', color: 'var(--gold)', border: 'none', borderRadius: '3px', fontFamily: 'var(--mono)', fontSize: '9px', letterSpacing: '.15em', textTransform: 'uppercase', cursor: 'pointer' }}>+ Adicionar Post</button>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', padding: '10px 16px', background: '#fff', border: '1px solid var(--border)', borderRadius: '4px', boxShadow: 'var(--shadow)', flexWrap: 'wrap', gap: '10px' }}>
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: '9px', color: 'var(--muted)', letterSpacing: '.1em', marginTop: '2px' }}>{wkPosts.length} post{wkPosts.length !== 1 ? 's' : ''} nesta semana</div>
        </div>
        <button onClick={() => changeWk(1)} style={{ background: 'transparent', border: '1px solid var(--border)', borderRadius: '3px', color: 'var(--bronze)', cursor: 'pointer', fontSize: '16px', padding: '4px 12px' }}>›</button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '12px', marginBottom: '20px' }}>
        <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: '4px', padding: '14px', boxShadow: 'var(--shadow)' }}>
          <div style={{ fontSize: '8.5px', letterSpacing: '.2em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '10px' }}>Detalhes de Engajamento</div>
          {[
            { label: 'Curtidas', val: totalCurtidas, icon: '❤️' },
            { label: 'Comentários', val: totalComentarios, icon: '💬' },
            { label: 'Salvamentos', val: totalSalvamentos, icon: '🔖' },
          ].map(m => (
        ...
        {/* Notes */}
        <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: '4px', padding: '14px', boxShadow: 'var(--shadow)' }}>
          <div style={{ fontSize: '8.5px', letterSpacing: '.2em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '10px' }}>📝 Anotações da Semana</div>
      ...
      <div style={{ background: '#fff', borderRadius: '4px', border: '1px solid var(--border)', boxShadow: 'var(--shadow)', overflow: 'hidden' }}>
        <div style={{ padding: '13px 17px 11px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: '9px', letterSpacing: '.25em', textTransform: 'uppercase', color: 'var(--muted)' }}>Posts da Semana</span>
        </div>
        {wkPosts.length === 0 ? (
          <div style={{ padding: '32px', textAlign: 'center', color: 'var(--muted)', fontSize: '13px', fontStyle: 'italic' }}>
            Nenhum post nesta semana. Clique em "+ Adicionar Post" para começar.
          </div>
        ) : (
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  {['Data','Título','Formato','Alcance','Impressões','❤️','💬','🔖','Seguidores','Taxa Eng.',''].map(h => (
                ...
                {wkPosts.map(p => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding: '10px 12px', fontSize: '11px', color: 'var(--mocha)', borderBottom: '1px solid var(--border)', fontFamily: 'var(--prose)', fontWeight: 600, maxWidth: '200px' }}>{p.title}</td>
                      <td style={{ padding: '10px 12px', fontSize: '10px', color: 'var(--muted)', borderBottom: '1px solid var(--border)', whiteSpace: 'nowrap' }}>{p.format || '—'}</td>
                      <td style={{ padding: '10px 12px', fontSize: '11px', fontWeight: 500, color: 'var(--mocha)', borderBottom: '1px solid var(--border)' }}>{num(p.alcance).toLocaleString('pt-BR')}</td>
                      <td style={{ padding: '10px 12px', fontSize: '11px', color: 'var(--mocha)', borderBottom: '1px solid var(--border)' }}>{num(p.impressoes).toLocaleString('pt-BR')}</td>
                      <td style={{ padding: '10px 12px', fontSize: '11px', color: 'var(--mocha)', borderBottom: '1px solid var(--border)' }}>{num(p.curtidas).toLocaleString('pt-BR')}</td>
                      <td style={{ padding: '10px 12px', fontSize: '11px', color: 'var(--mocha)', borderBottom: '1px solid var(--border)' }}>{num(p.comentarios).toLocaleString('pt-BR')}</td>
                      <td style={{ padding: '10px 12px', fontSize: '11px', color: 'var(--mocha)', borderBottom: '1px solid var(--border)' }}>{num(p.salvamentos).toLocaleString('pt-BR')}</td>
                      ...
                      <td style={{ padding: '10px 12px', borderBottom: '1px solid var(--border)', whiteSpace: 'nowrap' }}>
                        ...
                        <button onClick={() => { if (window.confirm('Excluir este post?')) deletePost(p.id) }} style={{ background: 'transparent', border: 'none', color: 'var(--dusty)', cursor: 'pointer', fontSize: '11px' }}>✕</button>
                      </td>
                    </tr>
                  )
                })}
              ...
              {wkPosts.length > 1 && (
                <tfoot>
                  <tr style={{ background: 'var(--warm)' }}>
                    <td colSpan={3} style={{ padding: '10px 12px', fontSize: '8.5px', letterSpacing: '.15em', textTransform: 'uppercase', color: 'var(--muted)', borderTop: '1px solid var(--border)' }}>Total da Semana</td>
                    <td style={{ padding: '10px 12px', fontSize: '11px', fontWeight: 700, color: 'var(--mocha)', borderTop: '1px solid var(--border)' }}>{totalAlcance.toLocaleString('pt-BR')}</td>
                    <td style={{ padding: '10px 12px', fontSize: '11px', fontWeight: 700, color: 'var(--mocha)', borderTop: '1px solid var(--border)' }}>{totalImpressoes.toLocaleString('pt-BR')}</td>
                    <td style={{ padding: '10px 12px', fontSize: '11px', fontWeight: 700, color: 'var(--mocha)', borderTop: '1px solid var(--border)' }}>{totalCurtidas.toLocaleString('pt-BR')}</td>
                    <td style={{ padding: '10px 12px', fontSize: '11px', fontWeight: 700, color: 'var(--mocha)', borderTop: '1px solid var(--border)' }}>{totalComentarios.toLocaleString('pt-BR')}</td>
                    <td style={{ padding: '10px 12px', fontSize: '11px', fontWeight: 700, color: 'var(--mocha)', borderTop: '1px solid var(--border)' }}>{totalSalvamentos.toLocaleString('pt-BR')}</td>
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: '6px', padding: '30px', maxWidth: '520px', width: '100%', boxShadow: '0 20px 60px rgba(44,31,20,.2)', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              <div style={{ gridColumn: '1 / -1' }}>
                <label style={{ display: 'block', fontSize: '8px', letterSpacing: '.2em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '5px' }}>Título / Descrição</label>
              ...
              <div>
                <label style={{ display: 'block', fontSize: '8px', letterSpacing: '.2em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '5px' }}>Data de Publicação</label>
            ...
            <div style={{ borderTop: '1px solid var(--border)', marginTop: '16px', paddingTop: '16px' }}>
              <div style={{ fontSize: '8.5px', letterSpacing: '.2em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '12px' }}>Métricas do Post</div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '10px' }}>
                {[
                  { key: 'alcance', label: 'Alcance', placeholder: '0' },
                  { key: 'impressoes', label: 'Impressões', placeholder: '0' },
                  { key: 'curtidas', label: 'Curtidas ❤️', placeholder: '0' },
                  { key: 'comentarios', label: 'Comentários 💬', placeholder: '0' },
                  { key: 'salvamentos', label: 'Salvamentos 🔖', placeholder: '0' },
                  { key: 'seguidores_ganhos', label: 'Seguidores Ganhos 🌱', placeholder: '0' },
                ].map(f => (
                  <div key={f.key}>
                    ...
                    <input type="number" value={form[f.key]} onChange={e => setForm(p => ({ ...p, [f.key]: e.target.value }))} placeholder={f.placeholder} min="0"
                      style={{ width: '100%', padding: '9px 12px', border: '1px solid var(--border)', borderRadius: '3px', fontFamily: 'var(--mono)', fontSize: '11px', color: 'var(--mocha)', background: 'var(--warm)', outline: 'none' }} />
                  </div>
                ))}
              </div>
            ...
            {(num(form.curtidas) + num(form.comentarios) + num(form.salvamentos) > 0 || num(form.alcance) > 0) && (
              <div style={{ marginTop: '14px', padding: '10px 14px', background: 'var(--warm)', borderRadius: '3px', border: '1px solid var(--border)', display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
                <div style={{ fontSize: '9px', color: 'var(--muted)' }}>Engajamento total: <strong style={{ color: 'var(--bronze)' }}>{(num(form.curtidas) + num(form.comentarios) + num(form.salvamentos)).toLocaleString('pt-BR')}</strong></div>
                {num(form.alcance) > 0 && <div style={{ fontSize: '9px', color: 'var(--muted)' }}>Taxa de engajamento: <strong style={{ color: 'var(--bronze)' }}>{(((num(form.curtidas) + num(form.comentarios) + num(form.salvamentos)) / num(form.alcance)) * 100).toFixed(1)}%</strong></div>}
              </div>
            )}
