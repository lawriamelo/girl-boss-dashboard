...
export default function Membros() {
  ...
  async function sendInvite(e) {
    ...
    const { error } = await supabase.auth.signInWithOtp({ email: inviteEmail.trim(), options: { emailRedirectTo: `${window.location.origin}/reset-password` } })
    if (error) { setInviteMsg('Erro ao enviar convite.'); setInviteLoading(false); return }
  ...
  function openEdit(member) {
    ...
    setEditId(member.id); setEditModal(true)
  }

  async function saveEdit() {
    const { data } = await supabase.from('profiles').update({ name: editForm.name, type: editForm.type, contact: editForm.contact, status: editForm.status, progress: editForm.progress }).eq('id', editId).select().single()
  ...
  if (!isAdmin) {
    ...
    return (
      <div>
        <div className="page-hdr"><div><div className="eyebrow">Área de</div><div className="page-title">Membros</div></div></div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', alignItems: 'start' }}>
          <div className="card">
            <div className="card-hdr"><span className="card-hdr-title">👤 Meu Perfil</span></div>
          ...
          <div className="card">
            <div className="card-hdr"><span className="card-hdr-title">📌 Mural de Recados</span></div>
            <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: '10px', maxHeight: '400px', overflowY: 'auto' }}>
              {updates.length === 0 ? <div style={{ fontSize: '12px', color: 'var(--muted)', fontStyle: 'italic', textAlign: 'center', padding: '20px 0' }}>Nenhum recado ainda.</div>
                : updates.map(u => (
  ...
  return (
    <div>
      <div className="page-hdr">
        <div><div className="eyebrow">Área de</div><div className="page-title">Membros</div></div>
      ...
      <div style={{ background: 'var(--mocha)', borderRadius: '4px', border: '1px solid rgba(201,169,110,.12)', padding: '16px 20px', marginBottom: '20px' }}>
        ...
        <form onSubmit={sendInvite} style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          <input type="email" value={inviteEmail} onChange={e => setInviteEmail(e.target.value)} placeholder="email@exemplo.com" required
            style={{ flex: '1 1 200px', padding: '9px 14px', border: '1px solid rgba(201,169,110,.2)', borderRadius: '3px', background: 'rgba(255,255,255,.06)', fontFamily: 'var(--mono)', fontSize: '11px', color: 'var(--cream)', outline: 'none' }} />
        ...
        {inviteMsg && <div style={{ marginTop: '8px', fontSize: '10px', color: inviteMsg.startsWith('✓') ? 'var(--sage)' : '#c47070' }}>{inviteMsg}</div>}
        <div style={{ marginTop: '8px', fontSize: '9px', color: 'rgba(176,144,128,.45)', lineHeight: 1.6 }}>A membra receberá um email com link para criar a própria senha.</div>
      </div>
      ...
      <div className="members-layout">
        <div>
          ...
          {filteredMembers.length === 0 ? (
            ...
            <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '13px 17px', borderRadius: '4px', border: '1px solid var(--border)', background: 'var(--warm)', marginBottom: '9px', transition: 'all .2s', flexWrap: 'wrap' }}
        ...
        <div style={{ background: 'var(--mocha)', border: '1px solid rgba(201,169,110,.12)', borderRadius: '4px', overflow: 'hidden', position: 'sticky', top: '20px' }}>
          ...
          <div style={{ padding: '14px', display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '500px', overflowY: 'auto' }}>
            {updates.length === 0 ? <div style={{ fontSize: '11px', color: 'rgba(176,144,128,.5)', fontStyle: 'italic', textAlign: 'center', padding: '20px 0' }}>Nenhum recado ainda.</div>
              : updates.map(u => (
              ...
              >
                ...
                <div className="upd-act" style={{ position: 'absolute', top: '8px', right: '8px', display: 'flex', gap: '4px', opacity: 0, transition: 'opacity .2s' }}>
                  <button onClick={() => { setUpdateForm({ type: u.type, text: u.text }); setEditUpdateId(u.id); setUpdateModal(true) }} style={{ background: 'transparent', border: 'none', color: 'var(--gold)', cursor: 'pointer', fontSize: '11px' }}>✏️</button>
        ...
        <div onClick={() => { setUpdateModal(false); setUpdateForm(EMPTY_UPDATE); setEditUpdateId(null) }} style={{ position: 'fixed', inset: 0, background: 'rgba(44,31,20,.55)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', backdropFilter: 'blur(5px)' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: '6px', padding: '30px', maxWidth: '460px', width: '100%', boxShadow: '0 20px 60px rgba(44,31,20,.2)' }}>
            ...
            <div style={{ marginBottom: '12px' }}>
              ...
              <textarea value={updateForm.text} onChange={e => setUpdateForm(p => ({ ...p, text: e.target.value }))} placeholder="Ex: Aula ao vivo esta quinta às 20h" rows={4}
